...
def get_data(filters):
	...
	result = []
	non_group_accounts = [x for x in data if not x.get("is_group")]
	for d in non_group_accounts:

		if d.get("account") and d.get("has_value"):
			d["opening_balance"] = d["opening_debit"] - d["opening_credit"]
			d["net_difference"] = d["debit"] - d["credit"]
			d["closing_balance"] = d["closing_debit"] - d["closing_credit"]
		d["indent"] = 0
		
		result.append(d)
	
	# Let's sort using the account number
	result = sorted(result, key=lambda x: x.get("account"))
	return result
