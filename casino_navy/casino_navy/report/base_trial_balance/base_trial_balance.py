...
def get_rootwise_opening_balances(filters, report_type):
	...
	for d in gle:
		...
		opening[d.account]["opening_debit"] += flt(d.debit_in_account_currency)
		opening[d.account]["opening_credit"] += flt(d.credit_in_account_currency)
...
def get_opening_balance(
	...
	opening_balance = (
		...
		.where(
			...
			(closing_balance.account_currency == filters.presentation_currency)
		)
	...
	for d in accounts:
		...
		for entry in gl_entries_by_account.get(d.name, []):
			if cstr(entry.is_opening) != "Yes":
				d["debit"] += flt(entry.debit_in_account_currency)
				d["credit"] += flt(entry.credit_in_account_currency)
